...
def get_fingerprint():
    ...
    if finger.image_2_tz(1) != fingerprintlib.OK:
        return False
    print("Searching...")
    if finger.finger_fast_search() != fingerprintlib.OK:
        return False
    return True
...
def enroll_finger(location):
    ...
    result1 = finger.get_fpdata("char", 1)
    result2 = finger.get_fpdata("char", 2)
    ...
    if i == fingerprintlib.OK:
        ...
        if i == fingerprintlib.BADLOCATION:
            print("Bad storage location")
        elif i == fingerprintlib.FLASHERR:
    ...
    '''print(type(result2))
    finger.send_fpdata(result1, sensorbuffer="char", slot=1)
    time.sleep(1)
    finger.send_fpdata(result2, sensorbuffer="char", slot=2)'''
    return True
...
def update_template(location):
    ...
    for p in data['worker']:
        template1 = p['parmak1']
        template2 = p['parmak2']
    finger.send_fpdata(template1, "char", 1)
    finger.send_fpdata(template2, "char", 2)
    # print(template1)
    # print(template2)
...
while True:
    print("----------------")
    if finger.read_templates() != fingerprintlib.OK:
        raise RuntimeError("Failed to read templates")
    print("Fingerprint templates: ", finger.templates)
    if finger.count_templates() != fingerprintlib.OK:
        raise RuntimeError("Failed to read templates")
    print("Number of templates found: ", finger.template_count)
    if finger.read_sysparam() != fingerprintlib.OK:
    ...
    if c == "u":
        update_template(get_num(finger.library_size))
    if c == "f":
        if get_fingerprint():
    ...
    if c == "d":
        if finger.delete_model(get_num(finger.library_size)) == fingerprintlib.OK:
            print("Deleted!")
        else:
    ...
    if c == "r":
        if finger.empty_library() == fingerprintlib.OK:
            print("Library empty!")
        else:
